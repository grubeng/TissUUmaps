...
flask.init = function () {
  $(document).on("click", ".layerSettingButton", function () {
    ...
    interfaceUtils.alert(
      `
            <iframe src='${path}/${$(this).data(
              "source"
            )}/info' style="width:100%;min-height:500px;"></iframe>
            `,
      "Image information"
    );
  });
...
flask.standalone.init = function () {
  ...
  // Add layer button
  div = HTMLElementUtils.createElement({
    kind: "div",
    extraAttributes: { class: "px-3 my-2" },
  });
  button = HTMLElementUtils.createElement({
    kind: "div",
  ...
  flask.standalone.pixelFlickering = HTMLElementUtils.createElement({
    kind: "div",
    extraAttributes: {
      style:
        "position:absolute;right:0px;bottom:0px;width:1px;height:1px;line-height:1px;background-color:#FFFFFF",
    },
  });
  document.body.append(flask.standalone.pixelFlickering);
  function refreshAnimation() {
    if (
      flask.standalone.pixelFlickering.style.backgroundColor ==
      "rgb(255, 255, 255)"
    )
      flask.standalone.pixelFlickering.style.backgroundColor =
        "rgb(255, 255, 254)";
  ...
  var clone = plusButton.cloneNode(true);
  plusButton.replaceWith(clone);
  document.getElementById("plus-1-button").addEventListener(
    "click",
    function (event) {
  ...
  projectUtils.getActiveProject().then((state) => {
    ...
    setTimeout(function () {
      loadingModal = interfaceUtils.loadingModal(
        "Exporting to static web page"
      );
    }, 0);
    setTimeout(function () {
      flask.standalone.backend.exportToStatic(
...
flask.server.init = function () {
  ...
  interfaceUtils.addMenuItem(
    ["File", "Save project"],
    function () {
      var modalUID = "messagebox";
      projectUtils.getActiveProject().then((state) => {
        interfaceUtils
          .prompt("Save project under the name:", "NewProject", "Save project")
          .then((filename) => {
  ...
  interfaceUtils.addMenuItem(
    ...
    function () {
      ...
      content.innerHTML =
        "<iframe src='/filetree' width='100%' height='300px'></iframe>";
  ...
  button.addEventListener("click", function () {
    ...
    content = HTMLElementUtils.createElement({ kind: "div" });
    content.innerHTML =
      "<iframe src='/filetree?addlayer=1' width='100%' height='300px'></iframe>";
    interfaceUtils.generateModal("Open file", content, buttons, modalUID);
  });
};
