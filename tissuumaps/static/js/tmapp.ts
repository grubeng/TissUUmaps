...
tmapp.registerActions = function () {
  ...
  interfaceUtils.listen(
    ...
    "click",
    function () {
      regionUtils.exportRegionsToJSON();
    },
    false
  );
  interfaceUtils.listen(
    ...
    "click",
    function () {
      regionUtils.importRegionsFromJSON();
    },
    false
  );
  interfaceUtils.listen(
    op + "_export_regions_csv",
    "click",
    function () {
      regionUtils.pointsInRegionsToCSV();
    },
    false
  );
  interfaceUtils.listen(
  ...
  interfaceUtils.listen(
    ...
    "click",
    function () {
      interfaceUtils.generateDataTabUI();
    },
    false
  );
  interfaceUtils.listen(
  ...
  for (var i = 0; i < uls.length; i++) {
    ...
    for (var j = 0; j < as.length; j++) {
      as[j].addEventListener("click", function () {
        interfaceUtils.hideTabsExcept($(this));
      });
    }
  }
...
tmapp.init = function () {
  ...
  overlayUtils._d3nodes[op + "_markers_svgnode"] = overlayUtils._d3nodes[
    op + "_svgnode"
  ]
    .append("g")
    .attr("id", op + "_markers_svgnode");
  //overlay for region data                                              //main node
  overlayUtils._d3nodes[op + "_regions_svgnode"] = overlayUtils._d3nodes[
    op + "_svgnode"
  ]
    .append("g")
  ...
  overlayUtils._d3nodes[cpop + "_svgnode"] = overlayUtils._d3nodes[
    op + "_svgnode"
  ]
    .append("g")
  ...
  tmapp["ISS_viewer"].addHandler(
    "animation-finish",
    function animationFinishHandler(event) {
      d3.selectAll("." + regionUtils._drawingclass)
        .selectAll("polyline")
        .each(function (el) {
          $(this).attr(
            "stroke-width",
