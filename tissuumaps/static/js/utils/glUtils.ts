...
glUtils._loadShaderProgram = function (
  gl,
  vertSource,
  fragSource,
  definitions = ""
...
glUtils.loadMarkers = function (uid, forceUpdate) {
  ...
  const colorPropertyName = (newInputs.colorPropertyName =
    dataUtils.data[uid]["_cb_col"]);
  const useColorFromMarker = (newInputs.useColorFromMarker =
    dataUtils.data[uid]["_cb_col"] != null &&
    dataUtils.data[uid]["_cb_cmap"] == null);
  let hexColor = "#000000";

  const scalarPropertyName = (newInputs.scalarPropertyName =
    dataUtils.data[uid]["_cb_col"]);
  const colorscaleName = dataUtils.data[uid]["_cb_cmap"];
  const useColorFromColormap = (newInputs.useColorFromColormap =
    dataUtils.data[uid]["_cb_cmap"] != null);
  let scalarRange = glUtils._markerScalarRange[uid];

  const scalePropertyName = (newInputs.scalePropertyName =
    dataUtils.data[uid]["_scale_col"]);
  const useScaleFromMarker = (newInputs.useScaleFromMarker =
    dataUtils.data[uid]["_scale_col"] != null);
  const markerScaleFactor = dataUtils.data[uid]["_scale_factor"];

  const markerCoordFactor = (newInputs.markerCoordFactor =
    dataUtils.data[uid]["_coord_factor"]);

  const sectorsPropertyName = (newInputs.sectorsPropertyName =
    dataUtils.data[uid]["_pie_col"]);
  ...
  if (dataUtils.data[uid]["_pie_dict"] && sectorsPropertyName) {
    ...
    for (let i = 0; i < sectorNames.length; ++i) {
      const key = Array.isArray(dataUtils.data[uid]["_pie_dict"])
        ? i
        : sectorNames[i];
  ...
  const numSectors = usePiechartFromMarker
    ? markerData[sectorsPropertyName][0].toString().split(";").length
    : 1;

  const shapePropertyName = (newInputs.shapePropertyName =
    dataUtils.data[uid]["_shape_col"]);
  const useShapeFromMarker = (newInputs.useShapeFromMarker =
    dataUtils.data[uid]["_shape_col"] != null);
  const numShapes = Object.keys(markerUtils._symbolStrings).length;
  let shapeIndex = 0;

  const opacityPropertyName = (newInputs.opacityPropertyName =
    dataUtils.data[uid]["_opacity_col"]);
  const useOpacityFromMarker = (newInputs.useOpacityFromMarker =
    dataUtils.data[uid]["_opacity_col"] != null);
  ...
  const collectionItemPropertyName = (newInputs.collectionItemPropertyName =
    dataUtils.data[uid]["_collectionItem_col"]);
  const useCollectionItemFromMarker = (newInputs.useCollectionItemFromMarker =
    dataUtils.data[uid]["_collectionItem_col"] != null);
  const collectionItemFixed = (newInputs.collectionItemFixed =
    dataUtils.data[uid]["_collectionItem_fixed"]);
  let collectionItemIndex = collectionItemFixed;

  const sortByCol = (newInputs.sortByCol = dataUtils.data[uid]["_sortby_col"]);
  const useSortByCol = (newInputs.useSortByCol =
    dataUtils.data[uid]["_sortby_col"] != null);
  const sortByDesc = (newInputs.sortByDesc =
    dataUtils.data[uid]["_sortby_desc"]);
  ...
  if (
    forceUpdate ||
    lastInputs != JSON.stringify(newInputs) ||
    dataUtils.data[uid]["modified"]
  ) {
    ...
    for (let offset = 0; offset < numPoints; offset += chunkSize) {
      ...
      if (usePiechartFromMarker) {
        ...
        for (let i = 0; i < chunkSize; ++i) {
          ...
          const sectors = markerData[sectorsPropertyName][markerIndex]
            .toString()
            .split(";");
          const piechartAngles = glUtils._createPiechartAngles(sectors);
          const lutIndex =
            keyName != null
              ? barcodeToLUTIndex[markerData[keyName][markerIndex]]
              : 0;
          const opacity = useOpacityFromMarker
            ? markerData[opacityPropertyName][markerIndex]
            : 1.0;
          if (useCollectionItemFromMarker)
            collectionItemIndex =
              markerData[collectionItemPropertyName][markerIndex];
          ...
          for (let j = 0; j < numSectors; ++j) {
            ...
            bytedata_point[4 * k + 0] =
              markerData[xPosName][markerIndex] * markerCoordFactor;
            bytedata_point[4 * k + 1] =
              markerData[yPosName][markerIndex] * markerCoordFactor;
            ...
            bytedata_scale[k] = useScaleFromMarker
              ? markerData[scalePropertyName][markerIndex]
              : 1.0;
            bytedata_shape[k] =
              Math.floor(
                (j < numSectors - 1 ? piechartAngles[j + 1] : 0.0) * 4095.0
              ) +
              Math.floor(piechartAngles[j] * 4095.0) * 4096.0;
            bytedata_opacity[k] = Math.floor(
              Math.max(0.0, Math.min(1.0, opacity)) * 65535.0
            );
        ...
        for (let i = 0; i < chunkSize; ++i) {
          ...
          if (useColorFromMarker)
            hexColor = markerData[colorPropertyName][markerIndex];
          if (useColorFromColormap) {
            ...
            scalarRange[0] = Math.min(
              scalarRange[0],
              isNaN(scalarValue) ? Infinity : scalarValue
            );
            scalarRange[1] = Math.max(
              scalarRange[1],
              isNaN(scalarValue) ? -Infinity : scalarValue
            );
          }
          if (useShapeFromMarker) {
            ...
            if (isNaN(shapeIndex))
              shapeIndex = markerUtils._symbolStrings.indexOf(shapeIndex);
            shapeIndex =
              Math.max(0.0, Math.floor(Number(shapeIndex))) % numShapes;
          }

          bytedata_point[4 * i + 0] =
            markerData[xPosName][markerIndex] * markerCoordFactor;
          bytedata_point[4 * i + 1] =
            markerData[yPosName][markerIndex] * markerCoordFactor;
          bytedata_point[4 * i + 2] = lutIndex + Number(shapeIndex) * 4096.0;
          bytedata_point[4 * i + 3] = useColorFromColormap
            ? Number(scalarValue)
            : Number("0x" + hexColor.substring(1, 7));
          bytedata_index[i] = markerIndex; // Store index needed for picking
          bytedata_scale[i] = useScaleFromMarker
            ? markerData[scalePropertyName][markerIndex]
            : 1.0;
          bytedata_opacity[i] = Math.floor(
            Math.max(0.0, Math.min(1.0, opacity)) * 65535.0
          );
      ...
      if (!(uid + "_markers" in glUtils._buffers)) {
        document
          .getElementById(uid + "_menu-UI")
          .addEventListener("input", glUtils.updateColorLUTTextures);
        document
          .getElementById(uid + "_menu-UI")
      ...
      if (!(uid + "_markers" in glUtils._buffers))
        glUtils._buffers[uid + "_markers"] = glUtils._createVertexBuffer(
          gl,
          numPoints * numSectors * NUM_BYTES_PER_MARKER
        );
      if (!(uid + "_markers_secondary" in glUtils._buffers))
        glUtils._buffers[uid + "_markers_secondary"] =
          glUtils._createVertexBuffer(
            gl,
            numPoints * numSectors * NUM_BYTES_PER_MARKER_SECONDARY
          );
      if (!(uid + "_markers_indices" in glUtils._buffers))
        glUtils._buffers[uid + "_markers_indices"] = glUtils._createIndexBuffer(
          gl,
          numPoints * numSectors * 4
        ); // Use 32-bits indices
      ...
      if (!(uid + "_colorLUT" in glUtils._textures))
        glUtils._textures[uid + "_colorLUT"] =
          glUtils._createColorLUTTexture(gl);
      if (!(uid + "_colorscale" in glUtils._textures))
        glUtils._textures[uid + "_colorscale"] =
          glUtils._createColorScaleTexture(gl);
      ...
      if (offset == 0) {
        // If the number of sectors used is changed, we have to reallocate the buffers
        {
          ...
          const oldBufferSize = gl.getBufferParameter(
            gl.ARRAY_BUFFER,
            gl.BUFFER_SIZE
          );
        ...
        }
        {
          gl.bindBuffer(
            ...
            glUtils._buffers[uid + "_markers_indices"]
          );
      ...
      gl.bufferSubData(
        gl.ARRAY_BUFFER,
        (POINT_OFFSET + offset * 16) * numSectors,
        bytedata_point
      );
      gl.bindBuffer(
        gl.ARRAY_BUFFER,
      ...
      gl.bufferSubData(
        gl.ARRAY_BUFFER,
        (INDEX_OFFSET + offset * 4) * numSectors,
        bytedata_index
      );
      gl.bufferSubData(
        gl.ARRAY_BUFFER,
        (SCALE_OFFSET + offset * 4) * numSectors,
        bytedata_scale
      );
      gl.bufferSubData(
        gl.ARRAY_BUFFER,
        (SHAPE_OFFSET + offset * 4) * numSectors,
        bytedata_shape
      );
      gl.bufferSubData(
        gl.ARRAY_BUFFER,
        (OPACITY_OFFSET + offset * 2) * numSectors,
        bytedata_opacity
      );
      gl.bufferSubData(
        gl.ARRAY_BUFFER,
        (TRANSFORM_OFFSET + offset * 2) * numSectors,
        bytedata_transform
      );
      gl.bindBuffer(gl.ARRAY_BUFFER, null);
    }
    ...
    console.time("Generate index data");
    {
      ...
      if (useSortByCol) {
        ...
        if (sortByDesc) {
          ...
          bytedata_indices.sort(
            (i, j) => Number(colData[j]) - Number(colData[i])
          );
        } else {
          // Sort in ascending order
          bytedata_indices.sort(
            (i, j) => Number(colData[i]) - Number(colData[j])
          );
        }
      }
  ...
  if (useColorFromColormap) {
    glUtils._updateColorScaleTexture(
      gl,
      uid,
      glUtils._textures[uid + "_colorscale"]
    );
...
glUtils._loadEdges = function (uid, forceUpdate) {
  ...
  const connectionsPropertyName = (newInputs.connectionsPropertyName =
    dataUtils.data[uid]["_edges_col"]);
  ...
  if (markerData[connectionsPropertyName] != null) {
    for (let markerIndex = 0; markerIndex < numPoints; ++markerIndex) {
      const edges = markerData[connectionsPropertyName][markerIndex]
        .toString()
        .split(";");
  ...
  if (
    markerData[connectionsPropertyName] != null &&
    ...
    for (let offset = 0; offset < numPoints; offset += chunkSize) {
      ...
      for (let i = 0; i < chunkSize; ++i) {
        ...
        for (let j = 0; j < edges.length; ++j) {
          ...
          let lutIndex_j =
            keyName != null
              ? barcodeToLUTIndex[markerData[keyName][markerIndex_j]]
              : 0;
          let collectionItemIndex_j = collectionItemFixed;
          if (useCollectionItemFromMarker)
            collectionItemIndex_j =
              markerData[collectionItemPropertyName][markerIndex_j];
          ...
          bytedata_point[4 * k + 1] =
            markerData[yPosName][markerIndex] * markerCoordFactor;
          bytedata_point[4 * k + 2] =
            markerData[xPosName][markerIndex_j] * markerCoordFactor;
          bytedata_point[4 * k + 3] =
            markerData[yPosName][markerIndex_j] * markerCoordFactor;
          bytedata_index[k] = lutIndex + lutIndex_j * 4096.0;
          bytedata_opacity[k] = Math.floor(
            Math.max(0.0, Math.min(1.0, opacity)) * 65535.0
          );
          bytedata_transform[k] =
            collectionItemIndex + collectionItemIndex_j * 256;
        }
      ...
      if (!(uid + "_edges" in glUtils._buffers))
        glUtils._buffers[uid + "_edges"] = glUtils._createVertexBuffer(
          gl,
          numEdges * NUM_BYTES_PER_EDGE
        );
      ...
      gl.bufferSubData(
        gl.ARRAY_BUFFER,
        POINT_OFFSET + offsetEdges * 16,
        bytedata_point
      );
      gl.bufferSubData(
        gl.ARRAY_BUFFER,
        INDEX_OFFSET + offsetEdges * 4,
        bytedata_index
      );
      gl.bufferSubData(
        gl.ARRAY_BUFFER,
        OPACITY_OFFSET + offsetEdges * 2,
        bytedata_opacity
      );
      gl.bufferSubData(
        gl.ARRAY_BUFFER,
        TRANSFORM_OFFSET + offsetEdges * 2,
        bytedata_transform
      );
      gl.bindBuffer(gl.ARRAY_BUFFER, null);
...
glUtils._updateBarcodeToLUTIndexDict = function (uid, markerData, keyName) {
  ...
  console.log(
    "barcodeToLUTIndex, barcodeToKey",
    barcodeToLUTIndex,
    barcodeToKey
  );
};
...
glUtils._updateColorLUTTexture = function (gl, uid, texture) {
  ...
  for (let [barcode, index] of Object.entries(
    glUtils._barcodeToLUTIndex[uid]
  )) {
    ...
    colors[4 * index + 3] =
      Number(visible) * (1 - Number(hidden)) * (Number(shapeIndex) + 1);
  }
...
glUtils._updateTransformUBO = function (buffer) {
  ...
  for (let i = 0; i < tmapp["ISS_viewer"].world.getItemCount(); ++i) {
    ...
    imageOrientation =
      imageOrientation - Math.floor(imageOrientation / 360.0) * 360.0;
    ...
    imageTransforms[i * 8 + 2] =
      shiftX -
      k0 * (scaleX * imageWidth) * Math.cos(theta) +
      k1 * (scaleX * imageHeight) * Math.sin(theta);
    imageTransforms[i * 8 + 4] = flip * Math.sin(theta) * scaleY;
    imageTransforms[i * 8 + 5] = Math.cos(theta) * scaleY;
    imageTransforms[i * 8 + 6] =
      shiftY -
      k0 * (scaleY * imageWidth) * Math.sin(theta) -
      k1 * (scaleY * imageHeight) * Math.cos(theta);
  }
...
glUtils._updateRegionDataSplitTexture = function (
  gl,
  texture,
  collectionIndex = 0
) {
...
glUtils._updateRegionLUTTexture = function (gl, texture) {
  ...
  if (regionUtils._regionToColorLUT.length <= texeldata.length) {
    ...
    console.warn(
      "Color lookup table for regions exceeds allocated texture size"
    );
  }
...
glUtils._updateColorScaleTexture = function (gl, uid, texture) {
  ...
  for (let i = 0; i < 256; ++i) {
    ...
    if (
      ...
      colors[4 * i + 0] =
        Math.max(0.0, Math.min(1.0, r * (1.0 - 0.5 * b * b) + s * s)) * 255.99;
      colors[4 * i + 1] =
        Math.max(0.0, Math.min(1.0, g * (1.0 - r * r * b * b))) * 255.99;
      colors[4 * i + 2] =
...
glUtils._updateColorbarCanvas = function (resolution) {
  ...
  for (let [uid, numPoints] of Object.entries(glUtils._numPoints)) {
    ...
    let propertyMin = propertyRange[0]
      .toPrecision(7)
      .replace(/\.([^0]+)0+$/, ".$1");
    let propertyMax = propertyRange[1]
      .toPrecision(7)
      .replace(/\.([^0]+)0+$/, ".$1");
    ...
    const tabName = interfaceUtils.getElementById(
      uid + "_marker-tab-name"
    ).textContent;
    ...
    ctx.fillText(
      ...
      (18 + 1 + yOffset) * resolution
    );
    ctx.textAlign = "left";
    ctx.fillText(
      propertyMin,
      ctx.canvas.width / 2 - 128 * resolution + 1,
      (40 + 1 + yOffset) * resolution
    );
    ctx.textAlign = "right";
    ctx.fillText(
      propertyMax,
...
glUtils._createColorbarCanvas = function () {
  ...
  canvas.style =
    "position:relative; float:right; width:266px; bottom: 11px; right: 14px; " +
...
glUtils._drawColorPass = function (gl, viewportTransform, markerScaleAdjusted) {
  let drawOrder = [];
  for (let [uid, _] of Object.entries(glUtils._numPoints)) {
    drawOrder.push(uid);
  }
...
glUtils._drawMarkersByUID = function (
  gl,
  viewportTransform,
  markerScaleAdjusted,
  uid
  ...
  const program =
    glUtils._programs[useInstancing ? "markers_instanced" : "markers"];
  ...
  gl.uniform2fv(gl.getUniformLocation(program, "u_canvasSize"), [
    gl.canvas.width,
    gl.canvas.height,
  ]);
  gl.uniform1f(
  ...
  gl.uniform1f(
    gl.getUniformLocation(program, "u_maxPointSize"),
    useInstancing ? 2048 : 256
  );
  gl.uniformBlockBinding(
    program,
    gl.getUniformBlockIndex(program, "TransformUniforms"),
    0
  );
  ...
  gl.bindVertexArray(
    glUtils._vaos[uid + (useInstancing ? "_markers_instanced" : "_markers")]
  );

  // Set per-markerset uniforms
  gl.uniform1i(
    gl.getUniformLocation(program, "u_transformIndex"),
    glUtils._collectionItemIndex[uid] != null
      ? glUtils._collectionItemIndex[uid]
      : -1
  );
  gl.uniform1f(
    gl.getUniformLocation(program, "u_globalMarkerScale"),
    glUtils._globalMarkerScale * glUtils._markerScaleFactor[uid]
  );
  gl.uniform2fv(
    gl.getUniformLocation(program, "u_markerScalarRange"),
    glUtils._markerScalarRange[uid]
  );
  gl.uniform1f(
    gl.getUniformLocation(program, "u_markerOpacity"),
    glUtils._markerOpacity[uid]
  );
  gl.uniform1f(
    gl.getUniformLocation(program, "u_markerStrokeWidth"),
    glUtils._markerStrokeWidth[uid]
  );
  gl.uniform1i(
    gl.getUniformLocation(program, "u_markerFilled"),
    glUtils._markerFilled[uid]
  );
  gl.uniform1i(
    gl.getUniformLocation(program, "u_markerOutline"),
    glUtils._markerOutline[uid]
  );
  gl.uniform1i(
    gl.getUniformLocation(program, "u_useColorFromMarker"),
    glUtils._useColorFromMarker[uid]
  );
  gl.uniform1i(
    gl.getUniformLocation(program, "u_useColorFromColormap"),
    glUtils._useColorFromColormap[uid]
  );
  gl.uniform1i(
    gl.getUniformLocation(program, "u_usePiechartFromMarker"),
    glUtils._usePiechartFromMarker[uid]
  );
  gl.uniform1i(
    gl.getUniformLocation(program, "u_useShapeFromMarker"),
    glUtils._useShapeFromMarker[uid]
  );
  gl.uniform1i(
    gl.getUniformLocation(program, "u_pickedMarker"),
...
glUtils._drawEdgesByUID = function (
  gl,
  viewportTransform,
  markerScaleAdjusted,
  uid
  ...
  gl.uniform1f(
    gl.getUniformLocation(program, "u_maxPointSize"),
    glUtils._useInstancing ? 2048 : 256
  );
  gl.uniform1f(
    gl.getUniformLocation(program, "u_edgeThicknessRatio"),
    glUtils._edgeThicknessRatio
  );
  gl.uniformBlockBinding(
    program,
    gl.getUniformBlockIndex(program, "TransformUniforms"),
...
glUtils._drawRegionsColorPass = function (gl, viewportTransform) {
  ...
  for (let collectionIndex in regionUtils._edgeListsByLayer) {
    ...
    gl.uniform1i(
      gl.getUniformLocation(program, "u_numScanlines"),
      numScanlines
    );
    gl.uniform1f(
      gl.getUniformLocation(program, "u_regionOpacity"),
      glUtils._regionOpacity
    );
    gl.uniform1i(
      gl.getUniformLocation(program, "u_regionFillRule"),
      fillRuleConstants[glUtils._regionFillRule]
    );
    gl.uniform1i(
      gl.getUniformLocation(program, "u_regionUsePivotSplit"),
      glUtils._regionUsePivotSplit
    );
    gl.uniform1i(
      gl.getUniformLocation(program, "u_regionUseColorByID"),
      glUtils._regionUseColorByID
    );
    gl.uniformBlockBinding(
      program,
      gl.getUniformBlockIndex(program, "TransformUniforms"),
...
glUtils._drawPickingPass = function (
  gl,
  viewportTransform,
  markerScaleAdjusted
) {
  ...
  gl.uniform2fv(
    gl.getUniformLocation(program, "u_pickingLocation"),
    glUtils._pickingLocation
  );
  gl.uniform1f(
    gl.getUniformLocation(program, "u_markerScale"),
    markerScaleAdjusted
  ...
  for (let uid of drawOrder) {
    ...
    const picked =
      Number(
        result[0] + result[1] * 256 + result[2] * 65536 + result[3] * 16777216
      ) - 1;
    if (picked >= 0) glUtils._pickedMarker = [uid, picked];
  }
...
glUtils.draw = function () {
  ...
  if (glUtils._useMarkerScaleFix)
    markerScaleAdjusted *= gl.canvas.height / 900.0;
...
glUtils.pick = function (event) {
  if (event.quick) {
    ...
    glUtils._pickingLocation = [
      event.position.x * glUtils._resolutionScaleActual,
    ...
    if (hasPickedMarker && glUtils._showMarkerInfo) {
      ...
      const groupName =
        keyName != null ? markerData[keyName][markerIndex] : undefined;
      ...
      div.height = "1px";
      if (glUtils._usePiechartFromMarker[uid]) {
        div.innerHTML = markerUtils.makePiechartTable(
          uid,
          markerIndex,
          piechartPropertyName
        );
      } else {
      ...
      tmapp["ISS_viewer"].addOverlay({
        ...
        location: tmapp[
          "ISS_viewer"
        ].viewport.viewerElementToViewportCoordinates(event.position),
        checkResize: false,
        rotationMode: OpenSeadragon.OverlayRotationMode.NO_ROTATION,
      });
      interfaceUtils._mGenUIFuncs.ActivateTab(uid);
      var tr = document.querySelectorAll(
        '[data-uid="' + uid + '"][data-key="' + groupName + '"]'
      )[0];
      if (tr != null) {
...
glUtils.resize = function () {
  ...
  glUtils._resolutionScaleActual =
    glUtils._resolutionScale * window.devicePixelRatio;
  if (Math.max(width, height) * glUtils._resolutionScale >= 4096.0) {
    ...
    glUtils._resolutionScaleActual *=
      4096.0 / (Math.max(width, height) * glUtils._resolutionScale);
  }
...
glUtils.updateMarkerScale = function () {
  ...
  glUtils._markerScale = Math.max(
    0.01,
    Math.min(20.0, globalMarkerSize / 25.0)
  );
};
...
glUtils.restoreLostContext = function (event) {
  ...
  glUtils._programs["markers"] = glUtils._loadShaderProgram(
    gl,
    glUtils._markersVS,
    glUtils._markersFS
  );
  glUtils._programs["markers_instanced"] = glUtils._loadShaderProgram(
    gl,
    glUtils._markersVS,
    glUtils._markersFS,
    "#define USE_INSTANCING\n"
  );
  glUtils._programs["picking"] = glUtils._loadShaderProgram(
    gl,
    glUtils._pickingVS,
    glUtils._pickingFS
  );
  glUtils._programs["edges"] = glUtils._loadShaderProgram(
    gl,
    glUtils._edgesVS,
    glUtils._edgesFS
  );
  glUtils._programs["regions"] = glUtils._loadShaderProgram(
    gl,
    glUtils._regionsVS,
    glUtils._regionsFS
  );
  glUtils._textures["shapeAtlas"] = glUtils._loadTextureFromImageURL(
    gl,
...
glUtils.init = function () {
  ...
  if (!(gl instanceof WebGL2RenderingContext)) {
    interfaceUtils.alert(
      "Error: TissUUmaps requires a web browser that supports WebGL 2.0"
    );
  }
  ...
  this._programs["markers_instanced"] = this._loadShaderProgram(
    gl,
    this._markersVS,
    this._markersFS,
    "#define USE_INSTANCING\n"
  );
  this._programs["picking"] = this._loadShaderProgram(
    gl,
    this._pickingVS,
    this._pickingFS
  );
  this._programs["edges"] = this._loadShaderProgram(
    gl,
    this._edgesVS,
    this._edgesFS
  );
  this._programs["regions"] = this._loadShaderProgram(
    gl,
    this._regionsVS,
    this._regionsFS
  );
