...
dataUtils.processRawData = function (data_id, rawdata) {
  ...
  for (let i = 0; i < rawdata.columns.length; ++i) {
    ...
    if (rawdata.isnan[i]) {
      ...
      data_obj["_processeddata"][rawdata.columns[i]] = new Float64Array(
        numRows
      );
  ...
  let getH5Data = function (drop) {
    return new Promise((resolve, reject) => {
      ...
      if (alldrops[drop].value != "") {
        ...
        dataUtils._hdf5Api.getXRow(url, h5range, h5path).then(
          (data) => {
            if (
              Object.prototype.toString.call(data).includes("BigInt") ||
              Object.prototype.toString.call(data).includes("BigUint")
            ) {
 ...
 * @param {String} data_id The id of the data group like "U234345"
 */
dataUtils.updateViewOptions = async function (
  data_id,
  force_reload_all,
  reloadH5
) {
  try {
    ...
    let progressParent = interfaceUtils.getElementById(
      data_id + "_csv_progress_parent"
    );
    ...
    var _selectedOptions =
      interfaceUtils._mGenUIFuncs.areRadiosAndChecksChecked(data_id);
    ...
    if (tmapp["ISS_viewer"].world.getItemCount() == 0) {
      ...
      if (
        tmapp["ISS_viewer"].world.getItemAt(0).source.getTileUrl(0, 0, 0) ==
        null
      ) {
    ...
    if (recompute_background_img) {
      ...
      if (tmapp["ISS_viewer"].world.getItemCount() > 0) {
        if (
          tmapp["ISS_viewer"].world.getItemAt(0).source.height <
            parseInt(maxY * 1.06) ||
          tmapp["ISS_viewer"].world.getItemAt(0).source.width <
            parseInt(maxX * 1.06)
        ) {
          tmapp["ISS_viewer"].close();
          setTimeout(function () {
            dataUtils.updateViewOptions(data_id, false, false);
          }, 50);
          return;
        }
      } else {
        ...
        setTimeout(function () {
          dataUtils.updateViewOptions(data_id, true, false);
        }, 50);
        return;
      }
    }
    ...
    if (radios["cb_gr"].checked) {
      ...
      if (inputs["cb_col"].value != "null") {
        ...
        interfaceUtils.alert(
          "No color column selected. Impossible to update view."
        );
        return;
      }
    }
    // Use piecharts column
    data_obj["_pie_col"] = radios["pie_check"].checked
      ? inputs["pie_col"].value
      : null;
    data_obj["_pie_dict"] = inputs["pie_dict"].value
      ? JSON.parse(inputs["pie_dict"].value)
      : [];
    if (data_obj["_pie_col"] == "null") {
      interfaceUtils.alert(
        "No piechart column selected. Impossible to update view."
      );
      return;
    }
    data_obj["_edges_col"] = radios["edges_check"].checked
      ? inputs["edges_col"].value
      : null;
    if (data_obj["_edges_col"] == "null") {
      interfaceUtils.alert(
        "No edges column selected. Impossible to update view."
      );
      return;
    }
    data_obj["_sortby_col"] = radios["sortby_check"].checked
      ? inputs["sortby_col"].value
      : null;
    data_obj["_sortby_desc"] = radios["sortby_check"].checked
      ? radios["sortby_desc_check"].checked
      : null;
    data_obj["_z_order"] = parseFloat(inputs["z_order"].value);

    if (data_obj["_sortby_col"] == "null") {
      interfaceUtils.alert(
        "No sort by column selected. Impossible to update view."
      );
      return;
    }
    // Use scale colummn
    data_obj["_scale_col"] = radios["scale_check"].checked
      ? inputs["scale_col"].value
      : null;
    if (data_obj["_scale_col"] == "null") {
      interfaceUtils.alert(
        "No size column selected. Impossible to update view."
      );
      return;
    }
    ...
    // Use shape column
    data_obj["_shape_col"] = radios["shape_col"].checked
      ? inputs["shape_col"].value
      : null;
    if (data_obj["_shape_col"] == "null") {
      interfaceUtils.alert(
        "No shape column selected. Impossible to update view."
      );
      return;
    }
    // Use opacity column
    data_obj["_opacity_col"] = radios["opacity_check"].checked
      ? inputs["opacity_col"].value
      : null;
    if (data_obj["_opacity_col"] == "null") {
      interfaceUtils.alert(
        "No opacity column selected. Impossible to update view."
      );
      return;
    } // Use collection column
    data_obj["_collectionItem_col"] = radios["collectionItem_col"].checked
      ? inputs["collectionItem_col"].value
      : null;
    data_obj["_collectionItem_fixed"] = radios["collectionItem_col"].checked
      ? null
      : parseInt(inputs["collectionItem_fixed"].value);
    if (data_obj["_collectionItem_col"] == "null") {
      interfaceUtils.alert(
        "No collection item column selected. Impossible to update view."
      );
      return;
    }
    if (
      (data_obj["_collectionItem_col"] ||
        data_obj["_collectionItem_fixed"] > 0) &&
    ...
    if (interfaceUtils.getElementById("ISS_globalmarkersize"))
      interfaceUtils
        .getElementById("ISS_globalmarkersize")
    ...
    interfaceUtils.alert(
      "Error while updating view: <br><pre><code>" + stack + "</code></pre>"
    );
  }
};
...
dataUtils.createMenuFromCSV = function (data_id, datumExample) {
  ...
  if (data_obj["expectedHeader"]) {
    interfaceUtils._mGenUIFuncs.fillRadiosAndChecksIfExpectedCSV(
      data_id,
      data_obj["expectedRadios"]
    );
    interfaceUtils._mGenUIFuncs.fillDropDownsIfExpectedCSV(
      data_id,
      data_obj["expectedHeader"]
    );
  ...
  Papa.parse(thecsv, {
    ...
    complete: function (result) {
      if (rawdata.tmp.length > 0 && rawdata.tmp[0].length > 0) {
        ...
        for (let i = 0; i < rawdata.columns.length; ++i) {
          rawdata.data[i].push(
            rawdata.isnan[i] ? rawdata.tmp[i] : new Float64Array(rawdata.tmp[i])
          );
        }
        rawdata.tmp = rawdata.columns.map((x) => []); // Clear buffers
      }
    ...
    error: function () {
      interfaceUtils.alert(
        "Impossible to load csv file, please check relative path in the tmap file:<br/><code>" +
          thecsv +
          "</code>",
        "CSV loading error..."
      );
...
dataUtils.makeQuadTrees = function (data_id) {
  ...
  if (groupByCol) {
    var allgroups = d3
      .nest()
      .key(function (d) {
        return markerData[groupByCol][d];
      })
      .entries(indexData);
    ...
    for (var i = 0; i < allgroups.length; i++) {
      ...
      if (dataUtils._quadtreesEnabled) {
        ...
        if (dataUtils._quadtreesMethod == 0) {
          data_obj["_groupgarden"][treeKey] = d3
            .quadtree()
            .x(x)
            .y(y)
            .addAll(allgroups[i].values);
        } else {
          ...
          dataUtils._quadtreeAddAll(
            data_obj["_groupgarden"][treeKey],
            allgroups[i].values,
            maxDepth,
            useArrayLeaves
          );
      ...
      if (groupByColsName) {
        const treeName =
          data_obj["_processeddata"][groupByColsName][allgroups[i].values[0]] ||
          "";
...
dataUtils._quadtreeAddAll = function (tree, indices, maxDepth, useArrayLeaves) {
  ...
  for (let i = 0, d, x, y; i < n; ++i) {
    if (
      isNaN((x = +tree._x.call(null, (d = indices[i])))) ||
      isNaN((y = +tree._y.call(null, d)))
    )
      continue;
    if (x < x0) x0 = x;
    if (x > x1) x1 = x;
