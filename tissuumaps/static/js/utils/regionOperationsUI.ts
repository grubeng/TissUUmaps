...
regionUtils.addRegionToolbarUI = function () {
  let buttonsContainer = document.getElementById("region-toolbar-buttons");
  if (!buttonsContainer) {
    ...
    const multipleRegionOperations = [
      {
        name: "union",
        icon: `<img style="width: 30px; height: 20px;" src="static/misc/union.svg" />`,
        title: "Merge selected regions",
      },
      {
        name: "xor",
        icon: `<img style="width: 30px; height: 20px;" src="static/misc/difference.svg" />`,
        title: "XOR selected regions",
      },
      {
        name: "intersect",
        icon: `<img style="width: 30px; height: 20px;" src="static/misc/intersection.svg" />`,
        title: "Intersect selected regions",
      },
    ];
    ...
    const mainRegionOperationsButton = HTMLElementUtils.createElement({
      ...
      extraAttributes: {
        class:
          "btn lh-1 btn-primary m-1 px-2 py-0 dropdown-toggle only-selected only-two-selected",
        type: "button",
    ...
    multipleRegionOperations.forEach((operation) => {
      ...
      anchor.addEventListener("click", () => {
        regionUtils.regionsClipper(
          Object.values(regionUtils._selectedRegions),
          operation.name
        );
    ...
    const duplicateButton = HTMLElementUtils.createButton({
      extraAttributes: {
        class: "btn lh-1 btn-primary m-1 p-2 only-selected",
        title: "Duplicate",
      },
    });
    ...
    deleteButton.onclick = function () {
      const regions = Object.values(regionUtils._selectedRegions);
      regionUtils.deleteRegions(regions.map((region) => region.id));
      regionUtils.resetSelection();
    };
    ...
    const splitButton = HTMLElementUtils.createButton({
      extraAttributes: {
        class: "btn lh-1 btn-primary m-1 p-2 only-selected",
        title: "Split multipolygons into multiple regions",
      },
    });
    splitButton.onclick = function () {
      const regions = Object.values(regionUtils._selectedRegions);
      regionUtils.splitRegions(regions.map((region) => region.id));
      regionUtils.resetSelection();
    };
    ...
    const fillHolesButton = HTMLElementUtils.createButton({
      extraAttributes: {
        class: "btn lh-1 btn-primary m-1 p-2 only-selected",
        title: "Fill holes in regions",
      },
    });
    fillHolesButton.onclick = function () {
      const regions = Object.values(regionUtils._selectedRegions);
      regionUtils.fillHolesRegions(regions.map((region) => region.id));
    };
    ...
    const unselectButton = HTMLElementUtils.createButton({
      extraAttributes: {
        class: "btn lh-1 btn-primary m-1 p-2 only-selected",
        title: "Unselect all regions (escape)",
      },
    });
    ...
    const mainButton = HTMLElementUtils.createElement({
      kind: "button",
      extraAttributes: {
        class: "btn lh-1 btn-light m-1 p-2 me-0",
        type: "button",
        "data-Selected": "0",
        title: "Drawing tool",
      },
    ...
    const splitDropdownButton = HTMLElementUtils.createElement({
      ...
      extraAttributes: {
        class:
          "btn lh-1 btn-light m-1 p-2 ms-0 dropdown-toggle dropdown-toggle-split",
        type: "button",
        "data-bs-toggle": "dropdown",
        "aria-expanded": "false",
    ...
    paintingTools.forEach((tool) => {
      ...
      const anchor = HTMLElementUtils.createElement({
        ...
        extraAttributes: { class: "dropdown-item", href: "#" },
        innerHTML:
          "<i class='bi " + tool.icon + "'></i>&nbsp;-&nbsp;" + tool.title,
      });
      anchor.addEventListener("click", () => {
    ...
    const selectionButton = HTMLElementUtils.createButton({
      extraAttributes: {
        class: "btn lh-1 btn-light m-1 p-2",
        title: "Select regions (press shift to select multiple regions)",
      },
    });
    ...
    const zoomSelectedButton = HTMLElementUtils.createButton({
      extraAttributes: {
        class: "btn lh-1 btn-primary m-1 p-2 only-selected",
        title: "Zoom to selected regions",
      },
    });
    ...
    tmapp.ISS_viewer.addControl(buttonsContainer, {
...
regionUtils.resizeRegionsModal = async function (regions) {
  ...
  let button1 = HTMLElementUtils.createButton({
    extraAttributes: { class: "btn btn-primary mx-2" },
  });
  button1.innerText = "Apply";
  let button2 = HTMLElementUtils.createButton({
    extraAttributes: {
  ...
  label111 = HTMLElementUtils.createElement({
    kind: "label",
    extraAttributes: { for: "scale_value_" + modalUID },
  });
  label111.innerText = "Scale (in percents):";
  value112 = HTMLElementUtils.createElement({
    kind: "input",
    id: "scale_value_" + modalUID,
    extraAttributes: {
  ...
  divformcheck310 = HTMLElementUtils.createElement({
    kind: "div",
    extraAttributes: { class: "form-check" },
  });
  inputcheck310 = HTMLElementUtils.createElement({
    kind: "input",
    id: modalUID + "_preview",
    extraAttributes: {
  ...
  label311 = HTMLElementUtils.createElement({
    kind: "label",
    id: modalUID + "_preview-label",
    extraAttributes: { for: modalUID + "_preview" },
  });
...
regionUtils.dilateRegionsModal = async function (regions) {
  ...
  label111 = HTMLElementUtils.createElement({
    kind: "label",
    extraAttributes: { for: "dilate_value_" + modalUID },
  });
  label111.innerText = "Size in pixel (negative for erosion):";
  value112 = HTMLElementUtils.createElement({
    kind: "input",
    id: "dilate_value_" + modalUID,
    extraAttributes: {
      class: "form-text-input form-control",
      type: "number",
      value: 0,
  ...
  divformcheck210 = HTMLElementUtils.createElement({
    kind: "div",
    extraAttributes: { class: "form-check" },
  });
  inputcheck210 = HTMLElementUtils.createElement({
    kind: "input",
    id: modalUID + "_border",
  ...
  label211 = HTMLElementUtils.createElement({
    kind: "label",
    id: modalUID + "_border-label",
    extraAttributes: { for: modalUID + "_border" },
  });
