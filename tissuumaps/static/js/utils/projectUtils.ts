...
projectUtils.saveProject = function () {
  projectUtils.getActiveProject().then((state) => {
    interfaceUtils
      .prompt("Save project under the name:", "NewProject")
      .then((filename) => {
        state.filename = filename;

        var dataStr =
          "data:text/json;charset=utf-8," +
          encodeURIComponent(JSON.stringify(state, null, 4));
  ...
  return new Promise((resolve, reject) => {
    ...
    function makeButtons(callback) {
      ...
      projectUtils
        .makeButtonFromTab(
          uid,
          "The tab " + tabName + " is not saved as a button yet",
          "modalButton_" + uid
        )
  ...
  return new Promise((resolve, reject) => {
    ...
    button2.addEventListener("click", function (event) {
      ...
      var title = document.getElementById(
        "generateButtonTitle_" + modalUID
      ).value;
      var comment = document.getElementById(
        "generateButtonComment_" + modalUID
      ).value;
      if (!_exists) {
        interfaceUtils
          .confirm(
            "Warning, path doesn't seem reachable from the server. Check that all files are in the same folder.<br/><br/>Are you sure you want to continue?"
          )
    ...
    label111 = HTMLElementUtils.createElement({
      kind: "label",
      extraAttributes: { for: "generateButtonPath_" + modalUID },
    });
    label111.innerText = "Relative path to the csv file (on the server side)";
    file112 = HTMLElementUtils.createElement({
      kind: "input",
      id: "generateButtonPath_" + modalUID,
      extraAttributes: {
    ...
    label211 = HTMLElementUtils.createElement({
      kind: "label",
      extraAttributes: { for: "generateButtonTitle_" + modalUID },
    });
    label211.innerText = "Button inner text";
    select212 = HTMLElementUtils.createElement({
      kind: "input",
      id: "generateButtonTitle_" + modalUID,
      extraAttributes: {
        class: "form-text-input form-control",
        type: "text",
        value: "Download data",
    ...
    label311 = HTMLElementUtils.createElement({
      kind: "label",
      extraAttributes: { for: "generateButtonComment_" + modalUID },
    });
    label311.innerText =
      "Comment (will be displayed on the right of the button)";
    select312 = HTMLElementUtils.createElement({
      kind: "input",
      id: "generateButtonComment_" + modalUID,
      extraAttributes: {
        class: "form-text-input form-control",
        type: "text",
        value: "",
...
projectUtils.removeTabFromProject = function (dataset) {
  if (dataUtils.data[dataset].fromButton !== undefined) {
    let stateMarkerFile =
      projectUtils._activeState.markerFiles[dataUtils.data[dataset].fromButton];
    if (stateMarkerFile.autoLoad) {
      projectUtils._activeState.markerFiles.splice(
        dataUtils.data[dataset].fromButton,
        1
      );
...
projectUtils.makeButtonFromTabAux = function (
  dataset,
  csvFile,
  title,
  comment,
  ...
  dataUtils.data[dataset].fromButton =
    projectUtils._activeState.markerFiles.length - 1;
...
projectUtils.loadProjectFileFromServer = function (path) {
  $.getJSON(path, function (json) {
    ...
    interfaceUtils.alert("error: " + textStatus);
  });
};
...
projectUtils.loadProject = function (state) {
  ...
  if (state.markerFiles) {
    state.markerFiles.forEach(function (markerFile, buttonIndex) {
      ...
      if (
        Object.prototype.toString.call(markerFile.path) === "[object Array]" ||
        markerFile.dropdownOptions
      ) {
        interfaceUtils.createDownloadDropdownMarkers(markerFile);
      } else {
        interfaceUtils.createDownloadButtonMarkers(markerFile);
  ...
  if (state.regionFiles) {
    state.regionFiles.forEach(function (regionFile) {
      if (
        Object.prototype.toString.call(regionFile.path) === "[object Array]"
      ) {
  ...
  if (state.hideChannelRange) {
    overlayUtils.waitLayersReady().then(() => {
      document
        .getElementsByClassName("channelRange")[0]
  ...
  if (state.menuButtons) {
    state.menuButtons.forEach(function (menuButton, i) {
      if (
        Object.prototype.toString.call(menuButton.text) !== "[object Array]"
      ) {
        menuButton.text = [menuButton.text];
      }
      interfaceUtils.addMenuItem(menuButton.text, function () {
        window.open(menuButton.url, "_self").focus();
      });
    });
  }
...
projectUtils.loadLayers = function (state) {
  ...
  setTimeout(function () {
    ...
    if (state.boundingBox) {
      setTimeout(function () {
        tmapp[tmapp["object_prefix"] + "_viewer"].viewport.fitBounds(
    ...
    if (state.layerOpacities && state.layerVisibilities) {
      $(".visible-layers").prop("checked", true);
      $(".visible-layers").click();
      tmapp.layers.forEach(function (layer, i) {
...
projectUtils.convertOldMarkerFile = function (markerFile) {
  if (!markerFile.expectedHeader) markerFile.expectedHeader = {};
  ...
  if (!markerFile.expectedRadios) markerFile.expectedRadios = {};
  if (markerFile.expectedCSV.piechart) {
    ...
    markerFile.expectedRadios.pie_check = false;
  }
  if (markerFile.expectedCSV.color) {
    ...
    markerFile.expectedRadios.cb_col = false;
  }
  if (markerFile.expectedCSV.scale) {
    ...
    markerFile.expectedRadios.scale_check = false;
  }
  if (!markerFile.uid) markerFile.uid = "uniquetab";
  ...
  if (markerFile.settings) {
    ...
    for (setting of markerFile.settings) {
      ...
      if (
        setting.module == "markerUtils" &&
        setting.function == "_selectedShape"
      ) {
        dictSymbol = { 6: 6 };
        if (dictSymbol[setting.value])
          setting.value = dictSymbol[setting.value];
        markerFile.expectedHeader.shape_fixed =
          markerUtils._symbolStrings[setting.value];
      }
      if (
        setting.module == "markerUtils" &&
        setting.function == "_randomShape"
      ) {
      ...
      if (
        setting.module == "markerUtils" &&
        setting.function == "_colorsperkey"
      ) {
        markerFile.expectedRadios.cb_gr = true;
        markerFile.expectedRadios.cb_gr_rand = false;
      ...
      if (
        setting.module == "HTMLElementUtils" &&
        setting.function == "_colorsperiter"
      ) {
        markerFile.expectedRadios.cb_gr = true;
        markerFile.expectedRadios.cb_gr_rand = false;
        markerFile.expectedRadios.cb_gr_key = false;
      ...
      if (
        setting.module == "HTMLElementUtils" &&
        setting.function == "_colorsperbarcode"
      ) {
        markerFile.expectedRadios.cb_gr = true;
        markerFile.expectedRadios.cb_gr_rand = false;
        markerFile.expectedRadios.cb_gr_key = false;
...
projectUtils.addLegend = function (htmlContent) {
  ...
  if (document.getElementById("markerLegend") == undefined) {
    ...
    tmapp[tmapp["object_prefix"] + "_viewer"].addControl(elt, {
