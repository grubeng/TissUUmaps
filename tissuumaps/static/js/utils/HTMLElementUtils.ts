...
HTMLElementUtils.inputTypeCheckbox = function (params) {
  ...
  params["class"] || null
    ? checkbox.setAttribute("class", params["class"])
    : null;
...
HTMLElementUtils.inputTypeNumber = function (params) {
  ...
  params["class"] || null
    ? numberInput.setAttribute("class", params["class"])
    : null;
...
HTMLElementUtils.inputTypeRange = function (params) {
  ...
  params["class"] || null
    ? rangeInput.setAttribute("class", params["class"])
    : null;
...
HTMLElementUtils.selectTypeDropDown = function (params) {
  ...
  params["class"] || null
    ? select.setAttribute("class", params["class"])
    : null;
...
HTMLElementUtils.createPanel = function (params) {
  ...
  params.headingInnerText || null
    ? panel_heading.appendChild(
        document.createTextNode(params.headingInnerText)
      )
    : null;
  ...
  if (params.extraAttributes) {
    for (var attr in params.extraAttributes) {
      if (attr == "class")
        column.setAttribute(
          "class",
          column.getAttribute("class") + " " + params.extraAttributes[attr]
        );
      else column.setAttribute(attr, params.extraAttributes[attr]);
    }
  }
  return column;
};
...
HTMLElementUtils.determinsticHTMLColor = function (key) {
  ...
  if (ggroup.includes(maincolor)) {
    U = 0;
    V = 0;
  } else if (tugroup.includes(maincolor)) {
    U = 0;
    V = 255;
  } else if (cgroup.includes(maincolor)) {
    U = 255;
    V = 0;
  } else if (agroup.includes(maincolor)) {
    U = 255;
    V = 255;
  } else {
    U = 0;
  ...
  if (agroup.includes(second)) {
    U += 80;
    V += 80;
  }
  if (cgroup.includes(second)) {
    U += 80;
    V += -80;
  }
  if (ggroup.includes(second)) {
    U += -80;
    V += -80;
  }
  if (tugroup.includes(second)) {
    U += -80;
    V += 80;
  }
  ...
  if (agroup.includes(third)) {
    y += 35;
  }
  if (cgroup.includes(third)) {
    y += 35;
  }
  if (ggroup.includes(third)) {
    y -= 35;
  }
  if (tugroup.includes(third)) {
    y -= 35;
  }
