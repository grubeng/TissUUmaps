...
pluginUtils.startPlugin = function (pluginID, options) {
  ...
  script.onload = function () {
    if (!window[pluginID]) {
      interfaceUtils.alert(`Impossible to load plugins/${pluginID}.js file.`);
      return;
    }
    pluginTitle = window[pluginID]["name"];
    if (!pluginTitle) pluginTitle = pluginID;

    pluginDiv = interfaceUtils.addPluginAccordion(pluginID, pluginTitle);
    if (window[pluginID].parameters !== undefined) {
      pluginUtils.loadParameters(
        pluginID,
        pluginDiv,
        window[pluginID].parameters
      );
    }
...
pluginUtils.loadParameters = function (pluginID, pluginDiv, parameters) {
  ...
  for (var parameterName in parameters) {
    ...
    if (parameter.type == "section") {
      ...
      let row01 = HTMLElementUtils.createElement({
        kind: "p",
        extraAttributes: { class: "" },
      });
      ...
      if (parameter.collapsed) {
        ...
        row0?.setAttribute(
          "class",
          "collapse_button_transform border-bottom-0 p-1"
        );
      }
