...
overlayUtils.addAllLayers = function () {
  ...
  if (tmapp.fixed_file && tmapp.fixed_file != "") {
    tmapp.layers.unshift({
      name: tmapp.slideFilename,
      tileSource: tmapp.fixed_file,
    });
...
">
  ...
  var label11 = HTMLElementUtils.createElement({
    kind: "label",
    extraAttributes: {
      for: "setBackgroundColor",
      class: "input-group-text py-1 px-2 small",
    },
  });
...
overlayUtils.addLayerSettings = function (
  layerName,
  tileSource,
  layerIndex,
  checked
  ...
  if (!layerTable) {
    ...
    for (
      filterIndex = 0;
      filterIndex < filterUtils._filtersUsed.length;
      filterIndex++
    ) {
      filterHeaders +=
        "<th class='text-center'>" +
        filterUtils._filtersUsed[filterIndex] +
        "</th>";
    }
  ...
  if (layerIndex < 0 || checked) visible.checked = true;
  ...
  visible.addEventListener("change", function (ev) {
    ...
    var slider = document.querySelectorAll(
      '[layer="' + layer + '"][type="range"]'
    )[0];
...
overlayUtils.addLayerSlider = function () {
  if (document.getElementById("channelRangeInput") == undefined) {
    ...
    changeFun = function (ev) {
      ...
      $("#visible-layer-" + (channel - -1)).click();
      channelName = tmapp.layers[channel - -1].name;
      channelId = channelName.replace(".dzi", "");
      document.getElementById("channelValue").innerHTML =
        "Channel " + (channel - -2) + ": " + channelName;
      if (overlayUtils._linkMarkersToChannels) {
        ...
        if (document.getElementById("uniquetab_" + channelId + "_check")) {
          $(
            document.getElementById("uniquetab_" + channelId + "_check")
          ).click();
        } else {
...
overlayUtils.addLayer = function (layer, i, visible) {
  ...
  if (tmapp["ISS_viewer"].world.getItemCount() != 0) {
    if (
      tmapp["ISS_viewer"].world.getItemAt(0).source.getTileUrl(0, 0, 0) == null
    ) {
  ...
  if (layer.transform_matrix) {
    ...
    rotation =
      (Math.atan2(transform_matrix[1], transform_matrix[0]) * 360) /
      (2 * Math.PI);
    var shear_y =
      Math.atan2(transform_matrix[4], transform_matrix[1]) -
      Math.PI / 2 -
      (2 * Math.PI * rotation) / 360;
    var scale_x = Math.sqrt(
      transform_matrix[0] * transform_matrix[0] +
        transform_matrix[3] * transform_matrix[3]
    );
    var scale_y =
      Math.sqrt(
        transform_matrix[1] * transform_matrix[1] +
          transform_matrix[4] * transform_matrix[4]
      ) * Math.cos(shear_y);
    if (scale_x < 0) {
      scale_x = -scale_x;
      flip = true;
  ...
  tmapp[vname].addTiledImage({
    ...
    success: function (i) {
      ...
      layerNX = tmapp[op + "_viewer"].world
        .getItemAt(tmapp[op + "_viewer"].world.getItemCount() - 1)
        .getContentSize().x;
      tmapp[op + "_viewer"].world
        .getItemAt(tmapp[op + "_viewer"].world.getItemCount() - 1)
        .setWidth((scale * layerNX) / layer0X);
      var point = new OpenSeadragon.Point(x / layer0X, y / layer0X);
      tmapp[op + "_viewer"].world
        .getItemAt(tmapp[op + "_viewer"].world.getItemCount() - 1)
        .setPosition(point);
  ...
  overlayUtils.waitLayersReady().then(() => {
    if (projectUtils._activeState.collectionMode) {
      ...
      for (var i = 0; i < tmapp["ISS_viewer"].world._items.length; i++) {
        ...
        var layer0X = tmapp[op + "_viewer"].world
          .getItemAt(0)
          .getContentSize().x;
...
overlayUtils.modifyDisplayIfAny = function () {
  ...
  if (ymax > imageHeight / imageWidth) {
    ymax = imageHeight / imageWidth;
  }
...
overlayUtils.savePNG = function () {
  interfaceUtils
    .prompt(
      "Resolution for export (1 = screen resolution):<br/>High resolution can take time to load!</small>",
      "4",
      "Capture viewport",
      "number"
    )
    .then((resolution) => {
      ...
      overlayUtils.waitFullyLoaded().then(() => {
        overlayUtils.getCanvasPNG(resolution).then(() => {
          ...
          document
            .getElementById("ISS_viewer")
            .style.setProperty("visibility", "unset");
        });
      });
    });
};
...
overlayUtils.getCanvasPNG = function (tiling) {
    ...
    return new Promise((resolve, reject) => {
      ...
      overlayUtils.waitFullyLoaded().then(() => {
        overlayUtils.getCanvasCtx().then((ctx_offset) => {
          ...
          setTimeout(() => {
            getCanvasCtx_aux(index + 1, size, ctx, bounds).then((ctx) => {
              resolve(ctx);
              return;
            });
          }, 200);
  ...
  function add_colorbar(ctx, resolution) {
    var ctx_colorbar = document
      .querySelector("#colorbar_canvas")
      .getContext("2d");
    if (ctx_colorbar.canvas.classList.contains("d-none")) return ctx;
    // Set up CSS size.
    ctx_colorbar.canvas.style.width =
      ctx_colorbar.canvas.style.width || ctx_colorbar.canvas.width + "px";
    ctx_colorbar.canvas.style.height =
      ctx_colorbar.canvas.style.height || ctx_colorbar.canvas.height + "px";
  ...
  return new Promise((resolve, reject) => {
    if (tiling > 1) {
      ...
      var ctx_osd = document
        .querySelector(".openseadragon-canvas canvas")
        .getContext("2d");
      var ctx_webgl = document
        .querySelector("#gl_canvas")
        .getContext("webgl2", glUtils._options);
      canvas.width =
        tiling * Math.min(ctx_osd.canvas.width, ctx_webgl.canvas.width);
      canvas.height =
        tiling * Math.min(ctx_osd.canvas.height, ctx_webgl.canvas.height);
      var bounds = tmapp.ISS_viewer.viewport.getBounds();
...
overlayUtils.getCanvasCtx = function () {
  return new Promise((resolve, reject) => {
    ...
    var svgString = new XMLSerializer().serializeToString(
      document.querySelector(".openseadragon-canvas svg")
    );
