  ...
  _filters: {
    ...
    Colormap: {
      ...
      params: {
        type: "select",
        options: ["None"].concat(
          dataUtils._d3LUTs.map(function (str) {
    ...
    selectParams = {
      eventListeners: {
        change: function (e) {
          ...
          if (checked) filterUtils._filtersUsed.push(filterName);
          else
            filterUtils._filtersUsed = filterUtils._filtersUsed.filter(
...
filterUtils.applyFilterItems = function (calledItems) {
  ...
  overlayUtils.waitLayersReady().then(() => {
    if (
      JSON.stringify(filterUtils._lastFilters) ==
      JSON.stringify(filterUtils._filterItems)
    ) {
      return;
    }
    filters = [];
    for (const layer in filterUtils._filterItems) {
      ...
      filters.push({
        ...
        toReset:
          JSON.stringify(filterUtils._lastFilters[layer]) !=
          JSON.stringify(filterUtils._filterItems[layer]),
      });
    }
    ...
    Object.keys(filterUtils._filterItems).forEach(function (key, index) {
      filterUtils._lastFilters[key] = JSON.parse(
        JSON.stringify(filterUtils._filterItems[key])
      );
    });
  });
};
...
filterUtils.setRangesFromFilterItems = function () {
  ...
  for (const layer in filterUtils._filterItems) {
    for (
      ...
      filterRange = document.querySelector(
        '[filter="' + item.name + '"][layer="' + layer + '"]'
      );
      if (filterRange) {
        ...
        if (filterRange.type == "color") {
          ...
          function rgbToHex(rgb) {
            ...
            return (
              "#" +
              componentToHex(array[0]) +
              componentToHex(array[1]) +
              componentToHex(array[2])
            );
          }
          console.log(item.value);
          console.log(
            "setRangesFromFilterItems",
            item.value,
            rgbToHex(item.value)
          );
...
filterUtils.getFilterItems = function () {
  ...
  overlayUtils.waitLayersReady().then(() => {
    ...
    for (i = 0; i < filterInputsRanges.length; i++) {
      ...
      if (
        filterInputsRanges[i].type == "range" ||
        filterInputsRanges[i].type == "select-one"
      )
  ...
  overlayUtils.waitLayersReady().then(() => {
    ...
    for (i = 0; i < tmapp[op + "_viewer"].world.getItemCount(); i++) {
      tmapp[op + "_viewer"].world
        .getItemAt(i)
        .setCompositeOperation(filterUtils._compositeMode);
    }
  });
};
...
filterUtils.createHTMLFilter = function (params) {
  ...
  filterInput.setAttribute(
    "id",
    "filterInput-" + params.filter + "-" + params.layer
  );
  if (type != "color") {
    filterInput.setAttribute(
      "list",
      "filterDatalist-" + params.filter + "-" + params.layer
    );
