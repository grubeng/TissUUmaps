...
export class H5_API {
  ...
  loadPromise(url) {
    ...
    if (typeof url === "string" || url instanceof String)
      if (!_url.startsWith("https")) _url = h5Utils.relative_root + _url;
    this.worker.postMessage({
      id: id,
      action: "load",
      payload: { requestChunkSize, url: _url },
    });
...
export class H5AD_API extends H5_API {
    ...
    return new Promise((resolve) => {
      this.get(url, { path: path }, "attr").then((data_X) => {
        ...
        this.get(url, { path: path + "/indptr" }).then((indptr) => {
          ...
          this.get(url, { path: path + "/indices", slice: [[x1, x2]] }).then(
            (indices) => {
              this.get(url, { path: path + "/data", slice: [[x1, x2]] }).then(
                (data) => {
  ...
  getXRow_array(url, rowIndex, path) {
    return new Promise((resolve) => {
      this.get(url, {
        path: path,
        slice: [[], [parseInt(rowIndex), parseInt(rowIndex) + 1]],
      }).then((data_X) => {
          ...
          this.get(url, { path: path + "/indptr" }).then((indptr) => {
            this.get(url, { path: path + "/indices" }).then((indices) => {
              ...
              for (let i = 0; i < indptr.value.length; i++) {
                str_array.push(
                  indices.value
                    .slice(indptr.value[i], indptr.value[i + 1])
                    .join(";")
                );
              }
              resolve(str_array);
