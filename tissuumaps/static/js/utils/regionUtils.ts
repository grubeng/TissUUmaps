...
regionUtils.getLayerFromCoord = function (coordinates) {
  ...
  for (var i = 0; i < viewer.world.getItemCount(); i++) {
    ...
    if (
      imageCoord.x > 0 &&
      imageCoord.y > 0 &&
      imageCoord.x < tiledImage.getContentSize().x &&
      imageCoord.y < tiledImage.getContentSize().y
    ) {
      return i;
    }
  }
  ...
  for (var i = 0; i < globalPoints.length; i++) {
    ...
    for (var j = 0; j < globalPoints[i].length; j++) {
      ...
      for (var k = 0; k < globalPoints[i][j].length; k++) {
        ...
        let imageCoord = tiledImage.imageToViewportCoordinates(x, y, true);
 ...
 * @returns viewportPoints
 */
regionUtils.viewportPointsToGlobalPoints = function (
  viewportPoints,
  layerIndex
) {
  var op = tmapp["object_prefix"];
...
regionUtils.manager = function (event) {
  ...
  if (regionUtils._isNewRegion) {
    ...
    if (
      regionUtils.distance(nextpoint, regionUtils._currentPoints[0]) <
        (2 * regionUtils._epsilonDistance) /
          tmapp["ISS_viewer"].viewport.getZoom() &&
      count >= 2
    ) {
    ...
    regionobj
      ...
      .attr("stroke-width", strokeWstr)
      .attr("class", "region" + idregion)
      .attr("id", "handle-" + count.toString() + "-region" + idregion)
      .attr(
        "transform",
        "translate(" +
          nextpoint[0].toString() +
          "," +
          nextpoint[1].toString() +
          ") scale(" +
          regionUtils._scaleHandle +
          ")"
      )
...
regionUtils.closePolygon = function () {
  ...
  regionUtils.addRegion(
    ...
    null,
    "",
    regionUtils._currentLayerIndex
  );
  regionUtils._currentPoints = null;
...
regionUtils.geoJSON2regions = async function (geoJSONObjects) {
  ...
  for (let geoJSONObjIndex in geoJSONObjects) {
    ...
    if (geoJSONObj.geometry === undefined) geometry = geoJSONObj;
    else geometry = geoJSONObj.geometry;
    ...
    var geoJSONObjClass = "";
    var hexColor = "#ff0000";
    if (!geoJSONObj.properties) geoJSONObj.properties = {};
...
regionUtils.addRegion = function (
  points,
  regionid,
  color,
  regionClass,
...
regionUtils.searchTreeForPointsInBbox = function (
  quadtree,
  x0,
  y0,
  x3,
...
regionUtils.filterPointsInLayer = function (points, dataObj, collectionIndex) {
  ...
  if (useCollectionItemFromMarker) {
    ...
    const pointsInLayer = points.filter(
      (d) => collectionItemArray[d] == collectionIndex
    );
 ...
 *  @param {Object} options Tell the function
 *  @summary Search for points inside a particular region */
regionUtils.searchTreeForPointsInRegion = function (
  quadtree,
  x0,
  y0,
  x3,
  ...
  const pointInBbox = regionUtils.searchTreeForPointsInBbox(
    quadtree,
    x0,
    y0,
    x3,
  ...
  const pointInLayer = regionUtils.filterPointsInLayer(
    pointInBbox,
    dataObj,
    collectionIndex
  );
  ...
  if (countsInsideRegion) {
    regionUtils._regions[regionid].barcodeHistogram.push({
      key: quadtree.treeID,
      name: quadtree.treeName,
      count: countsInsideRegion,
    });
  }
  return pointsInside;
};
...
regionUtils.fillAllRegions = function () {
  glUtils._regionFillRule =
    glUtils._regionFillRule == "never" ? "nonzero" : "never";
...
regionUtils.zoomToRegions = function (regions) {
  ...
  for (const region of regions) {
    ...
    const viewportRect = image.imageToViewportRectangle(
      region._gxmin,
      region._gymin,
      region._gxmax - region._gxmin,
      region._gymax - region._gymin
    );
  ...
  const OSDRect = new OpenSeadragon.Rect(x0, y0, x3 - x0, y3 - y0);
...
regionUtils.updateAllRegionClassUI = function () {
  setTimeout(() => {
    ...
    let collapseButtons = document.getElementsByClassName(
      "collapse_button_regionClass"
    );
    for (let i = 0; i < collapseButtons.length; i++) {
      if (collapseButtons[i].getAttribute("aria-expanded") == "true") {
        uncollapsedElements.push(
          collapseButtons[i].getAttribute("data-bs-target")
        );
      }
    }
...
regionUtils.changeRegion = function (regionid) {
  ...
  if (document.getElementById(regionid + "_name_ta")) {
    ...
    var rPanel_hist = document.getElementById(
      op + escapedRegionID + "_tr_hist"
    );
    if (
      regionUtils._regions[regionid].regionClass !=
      document.getElementById(escapedRegionID + "_class_ta").value
    ) {
      if (document.getElementById(escapedRegionID + "_class_ta").value) {
        regionUtils._regions[regionid].regionClass = document.getElementById(
          escapedRegionID + "_class_ta"
        ).value;
    ...
    if (document.getElementById(escapedRegionID + "_name_ta").value) {
      regionUtils._regions[regionid].regionName = document.getElementById(
        escapedRegionID + "_name_ta"
      ).value;
    } else {
      regionUtils._regions[regionid].regionName = regionid;
    }
    var newregioncolor = document.getElementById(
      escapedRegionID + "_color_input"
    ).value;
...
regionUtils.getPointsInRegion = function (regionid) {
  ...
  for (var uid of allDatasets) {
    ...
    for (var codeIndex in allkeys) {
      ...
      var pointsInside = regionUtils.searchTreeForPointsInRegion(
        groupgarden,
...
regionUtils.setMode = function (mode) {
  let selectButtonIcon = document.getElementById("region_selection_button");
  let mainButtonIcon = document.getElementById("region_drawing_button");
...
regionUtils.freeHandManager = function (event) {
  function onCanvasRelease() {
    ...
    OSDViewer.removeHandler("canvas-drag", createRegionFromCanvasDrag);
    // Remove release handler
    OSDViewer.removeHandler("canvas-release", onCanvasRelease);
  ...
  function createRegionFromCanvasDrag(event) {
    ...
    if (regionUtils._isNewRegion) {
      ...
      regionUtils._currentRegionId += 1;
      regionUtils._currentLayerIndex =
        regionUtils.getLayerFromCoord(normCoords);
...
regionUtils.rectangleManager = function (event) {
  ...
  function onCanvasRelease() {
    ...
    // reset and stop here
    if (
      last_rectangle[0] == last_rectangle[2] ||
...
regionUtils.brushHover = function (event) {
  ...
  regionobj
    ...
    .attr(
      "r",
      (0.2 * regionUtils._handleRadius) / tmapp["ISS_viewer"].viewport.getZoom()
    )
    .attr("fill", "#ff000088")
    .attr("stroke", "#ff0000")
    .attr("stroke-width", 0)
  ...
  if (regions.length == 1) {
    ...
    regionobj
      ...
      .attr(
        "font-size",
        (0.02 / tmapp["ISS_viewer"].viewport.getZoom()).toString() + "px"
      )
...
regionUtils.brushManager = function (event) {
  function onCanvasRelease() {
    ...
    var canvas =
      overlayUtils._d3nodes[tmapp["object_prefix"] + "_regions_svgnode"].node();
    var drawingclass = regionUtils._drawingclass;
    var regionid = regionUtils._editedRegion
      ? regionUtils._editedRegion.id
      : "region" + regionUtils._currentRegionId.toString();
    var regionclass = regionUtils._editedRegion
      ? regionUtils._editedRegion.regionClass
      : "";
    d3.select("." + drawingclass).remove();
  ...
  function getBrushShape(x1, y1, x2, y2, brushSize) {
    ...
    const mergedPoints = regionUtils.clipperPolygons(
      [[[brushCircle1]], [[brushCircle2]], [[brushRectangle]]],
      "union"
    );
    return mergedPoints;
  }
  ...
  function createRegionFromCanvasDrag(event) {
    ...
    if (regionUtils._lastPoints) {
      if (
        regionUtils.distance(
          [normCoords.x, normCoords.y],
          [regionUtils._lastPoints.x, regionUtils._lastPoints.y]
        ) <
        regionUtils._epsilonDistance / tmapp["ISS_viewer"].viewport.getZoom()
      ) {
        return;
      }
    }
    ...
    if (previousPolygon.length == 0) {
      ...
      regionUtils._currentPoints = regionUtils.objectToArrayPoints(
        regionUtils.regionToLowerCase(
          regionUtils.clipperPolygons([previousPolygon, brushShape], operation)
        )
      );
    }
    regionUtils._lastPoints = normCoords;
...
regionUtils.regionsToJSON = function () {
  if (window.Blob) {
    var op = tmapp["object_prefix"];
    var jsonse = JSON.stringify(
      regionUtils.regions2GeoJSON(regionUtils._regions)
    );
  ...
  for (let regionID of Object.keys(regionUtils._regions)) {
    ...
    for (let subregion of region.globalPoints) {
      for (let points of subregion) {
        ...
        const lsb = Math.max(
          0,
          Math.min(63, Math.floor(xMin * (64.0 / imageBounds[2])))
        );
        const msb = Math.max(
          0,
          Math.min(63, Math.floor(xMax * (64.0 / imageBounds[2])))
        );
        ...
        const upper = Math.min(
          Math.floor(yMax / scanlineHeight + overlap),
          numScanlines - 1
        );
        ...
        for (let i = 0; i < numPoints; ++i) {
          ...
          const lower = Math.max(
            Math.floor(Math.min(v0.y, v1.y) / scanlineHeight - overlap),
            0
          );
          const upper = Math.min(
            Math.floor(Math.max(v0.y, v1.y) / scanlineHeight + overlap),
            numScanlines - 1
          );
