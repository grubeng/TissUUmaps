...
markerUtils.updatePiechartLegend = function () {
  ...
  let markerData = undefined,
    sectorsPropertyName = undefined,
    piechartPalette = undefined;
  ...
  let title = HTMLElementUtils.createElement({
    kind: "div",
    innerHTML: "<b>Piechart legend</b>",
  });
  elt.appendChild(title);
  elt.appendChild(table);

  let sectors = [];
  const numSectors = markerData[sectorsPropertyName][0]
    .toString()
    .split(";").length;
...
markerUtils.makePiechartTable = function (
  uid,
  markerIndex,
  sectorsPropertyName
) {
  ...
  let sectors = [];
  const numSectors = markerData[sectorsPropertyName][markerIndex]
    .toString()
    .split(";").length;
  if (sectorsPropertyName.split(";").length == numSectors) {
    sectors = sectorsPropertyName.split(";"); // Use sector labels from CSV header
  ...
  if (dataUtils.data[uid]["_tooltip_fmt"]) {
    outText +=
      "<b>" + markerUtils.getMarkerTooltip(uid, markerIndex) + ":</b><br/>";
  }
  let sectorValues = markerData[sectorsPropertyName][markerIndex]
    .toString()
    .split(";");
  let sortedSectors = [];
  sectors.forEach(function (sector, index) {
  ...
  sortedSectors.sort(function cmp(a, b) {
    return b[0] - a[0];
  });
  ...
  sortedSectors.forEach(function (sector) {
    outText +=
      ...
      piechartPalette[sector[2] % piechartPalette.length] +
      ";padding:3px;margin:2px;display: inline-block;'>" +
      sector[1] +
      ": " +
      (sector[0] * 100).toFixed(1) +
      " %</span> ";
  });
  return outText;
};
...
markerUtils.getMarkerTooltip = function (uid, markerIndex) {
  ...
  const nameVal =
    nameName != null ? markerData[nameName][markerIndex] : undefined;
